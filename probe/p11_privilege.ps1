# Compile and run probe/p11_privilege.cs, with the compiler that is already inside
# Windows. No SDK, no NuGet, no network.
#
# WHY THIS WRAPPER EXISTS. p11_privilege.cs produced numbers that are now quoted as
...
#
# What it answers: which limits survive without SeIncreaseBasePriorityPrivilege.
#
...
$src = Join-Path $here "p11_privilege.cs"
if (-not (Test-Path $src)) { throw "$src is missing" }

$csc = "$env:WINDIR\Microsoft.NET\Framework64\v4.0.30319\csc.exe"
if (-not (Test-Path $csc)) { throw "csc.exe not found at $csc" }

$out = Join-Path ([IO.Path]::GetTempPath()) ("offpeak-probe11-" + [Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $out | Out-Null
try {
    $exe = Join-Path $out "p11_privilege.exe"
